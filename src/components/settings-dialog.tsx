...
export function SettingsDialog() {
    const [open, setOpen] = useMenuTrigger(MenuActions.SETTINGS)
    const [section, setSection] = useState<Section>('appearance')

    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="sm:max-w-3xl p-0 gap-0 overflow-hidden" aria-describedby={undefined}>
                <div className="flex h-120">
                    <nav className="w-44 shrink-0 border-r bg-background-panel flex flex-col gap-1.5 p-3 pt-12">
                    ...
                    <VisuallyHidden.Root>
                        <DialogTitle className="text-2xl font-bold tracking-tight">{section}</DialogTitle>
                    </VisuallyHidden.Root>
                    <div className="flex-1 flex flex-col min-w-0 p-12">
                        {section === 'appearance' && <AppearanceSection />}
                        {section === 'license' && <LicenseSection onClose={() => setOpen(false)} />}
                    </div>
                </div>
            </DialogContent>
        </Dialog>
...
function LicenseSection({ onClose }: { onClose: () => void }) {
    const { isPro } = useLicense()

    const handleActivate = () => {
        onClose()
        setTimeout(() => emitMenuEvent(MenuActions.ACTIVATE_LICENSE), 150)
    }
